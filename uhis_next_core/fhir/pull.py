...
from uhis_next_core.api.fhir import _code_system_uri, _gender_from_fhir
...
def do_pull():
    ...
    frappe.db.set_value(
        "UHIS Settings", "UHIS Settings", "fhir_last_pull_dt",
...
def _upsert_encounter(r):
    ...
    frappe.get_doc({
        ...
        "client_uuid": uid,
        "case": case_id,
        "encounter_type": enc_type if enc_type in ("Routine", "Followup", "Emergency", "Referral") else "Routine",
...
def _upsert_observation(r):
    ...
    cases = frappe.get_all("Case", filters={"patient": patient_id},
                           fields=["name"], order_by="opened_on desc", limit=1)
    if not cases:
        return
