...
function _hist_load_from_db(doctype, docname, onDone) {
	frappe.call({
		method: "uhis_next_core.ai.chat.get_chat_thread",
...
function _hist_save(doctype, docname) {
	...
	frappe.call({
		method: "uhis_next_core.ai.chat.save_chat_thread",
...
window.uhis_chat_send = function(doctype, docname) {
	...
	frappe.call({
		method: "uhis_next_core.ai.chat.ask_ai_stream",
		...
		callback: function(r) {
			...
			poll_id = setInterval(function() {
				...
				frappe.call({
					method: "uhis_next_core.ai.chat.poll_chat_status",
					args: { session_id },
