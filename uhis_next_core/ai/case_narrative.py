...
def enqueue_narrative(case_name):
    ...
    frappe.enqueue(
        "uhis_next_core.ai.case_narrative.generate_narrative",
        case_name=case_name,
        queue="default",
        now=False,
    )
...
def _mark_summary_stale(doc, event):
    ...
    if case_name:
        frappe.db.set_value("Case", case_name, "ai_summary_generated_at", None,
...
def generate_narrative(case_name):
    ...
    frappe.db.set_value("Case", case_name, {
        "ai_summary":              stamped,
    ...
    if patient:
        frappe.db.set_value("Patient", patient, "ai_summary_generated_at", None,
                            update_modified=False)
        frappe.enqueue(
...
def _build_context(case_name):
    ...
    days_open = (
        (frappe.utils.getdate(today) - frappe.utils.getdate(case.opened_on)).days
    ...
    return {
        ...
        "conditions":           [{"concept": c.concept, "status": c.clinical_status}
                                  for c in conditions],
