...
def generate_patient_narrative(patient_name):
    ...
    frappe.db.set_value("Patient", patient_name, {
        "ai_summary":              stamped,
    ...
    if household:
        frappe.db.set_value("Household", household, "ai_summary_generated_at", None,
                            update_modified=False)
        frappe.enqueue(
...
def _build_patient_context(patient_name):
    ...
    cases = frappe.get_all(
        ...
        fields=["name", "programme", "status", "risk_level",
    ...
    active_progs = list({c.programme for c in cases
                         if c.status == "Active" and c.programme})
