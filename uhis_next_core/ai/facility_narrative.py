...
def enqueue_facility_narrative(facility_name):
    ...
    frappe.enqueue(
        "uhis_next_core.ai.facility_narrative.generate_facility_narrative",
...
def generate_facility_narrative(facility_name):
    ...
    frappe.db.set_value("Facility", facility_name, {
        "ai_summary":              stamped,
...
def _build_facility_context(facility_name):
    ...
    top_risk_factors = [
        f"{k}: {v} cases"
        for k, v in sorted(factor_counts.items(), key=lambda x: -x[1])[:5]
    ]

    # Pending referrals
    pending_referrals = frappe.db.count(
        "Referral",
        {"case": ["in", case_names] if case_names else ["=", ""],
         "status": ["in", ["Pending", "Accepted"]]},
    ) if case_names else 0
    ...
    recent_encounters = frappe.db.count(
        "Encounter",
        {"case": ["in", case_names] if case_names else ["=", ""],
         "encounter_dt": [">=", seven_days_ago]},
    ) if case_names else 0
    ...
    for c in high_risk_cases:
        pt_summary = frappe.db.get_value(
            "Patient", c.patient, ["ai_summary", "ai_summary_generated_at"],
            as_dict=True,
        )
