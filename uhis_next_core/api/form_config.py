...
def serialize_programme(programme_name):
    forms = frappe.get_all(
        ...
        fields=[
            "form_doctype", "sequence", "mandatory",
            "trigger_field", "trigger_operator", "trigger_value", "next_form",
        ],
...
def serialize_clinical_questions():
    ...
    questions = frappe.get_all(
        "Clinical Question",
        fields=[
            "name", "label", "form_group", "sequence", "fieldtype", "options",
            "mandatory", "clinical", "fhir_concept",
            "min_age_months", "max_age_months", "gender",
            "visibility_expression", "mandatory_expression",
        ],
    ...
    for q in questions:
        if q.clinical and not q.fhir_concept:
            frappe.throw(
                f"Clinical question '{q.name}' has clinical=1 but no FHIR concept mapping."
            )
        concept = None
        if q.fhir_concept:
            c = frappe.get_value(
                "Concept", q.fhir_concept, ["code_system", "code"], as_dict=True
            )
            if c:
                concept = {
