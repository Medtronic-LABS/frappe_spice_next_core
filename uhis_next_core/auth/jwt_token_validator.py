...
(e.g. uhis_next_core.auth.decorators.require_remote_auth) use only the public
...
class JWTTokenValidator:
	...
	def extract_user_id(self, authorization_header):
		...
		if self._remote_client:
			try:
				return self._remote_client.validate(token)
			except RemoteAuthError as e:
				raise JWTDecodeError("token rejected by remote auth service") from e
