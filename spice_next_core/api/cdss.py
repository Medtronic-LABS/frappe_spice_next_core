...
CDSS endpoint — clinical decision support for a single encounter.
  ...
  spice_next_core.api.cdss.recommend
  spice_next_core.api.cdss.suggest_symptoms
...
# The client sends the lowercase form_group-style code used throughout the
# rest of this API (see module docstring); `Clinical Question Programme`
# stores the Title Case Programme name. `programmes` is now the sole
# scoping field on Clinical Question — every row carries at least one --
# so this mapping is the only thing standing between the two vocabularies.
_PROGRAMME_CODE_TO_NAME = {
	"ncd": "NCD",
	"anc": "ANC",
	"pnc_mother": "PNC",
	"pnc_child": "PNC",
...
def _resolve_symptom_questions(programme):
	"""Symptom Clinical Questions scoped to `programme` (a lowercase
	form_group-style code), matched via the `programmes` child table."""
	fields = ["name", "label"]
	if not programme:
	...
	programme_name = _PROGRAMME_CODE_TO_NAME.get(programme, programme.upper())
	...
	names = set(
		...
		.where(CQP.programme == programme_name)
		.select(CQP.parent)
		.distinct()
		.run(pluck=True)
	)
	if not names:
		return []
	return frappe.get_all(
		"Clinical Question", filters={"name": ["in", list(names)]},
		fields=fields, order_by="sequence asc",
	)
