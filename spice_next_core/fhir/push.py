...
_FHIR_MAP = {
	"Patient": ("Patient", "spice_next_core.api.fhir.read_patient"),
	"Encounter": ("Encounter", "spice_next_core.api.fhir.read_encounter"),
	"Observation": ("Observation", "spice_next_core.api.fhir.to_fhir_observation"),
	"Case": ("EpisodeOfCare", "spice_next_core.api.fhir.to_episode_of_care"),
	"Condition": ("Condition", "spice_next_core.api.fhir.read_condition"),
	"Referral": ("ServiceRequest", "spice_next_core.api.fhir.read_referral"),
}
...
def enqueue_fhir_push(doc, event):
	...
	if not cfg.fhir_sync_enabled or not cfg.fhir_server_url:
		return
	frappe.enqueue(
		"spice_next_core.fhir.push.do_push",
		doctype=doc.doctype,
		name=doc.name,
		queue="default",
		now=False,
