...
def advance_sync_seq(doc, event):
	...
	# Portable atomic increment: SELECT ... FOR UPDATE row-locks the counter
	# row for the rest of this transaction, so no concurrent request can read
	# a stale current_seq before this one's UPDATE commits. Works identically
	# on MariaDB/InnoDB and Postgres. (MariaDB's RETURNING clause only covers
	# INSERT/DELETE, never UPDATE, so that's not an option here.)
	current = frappe.db.sql(
		"SELECT current_seq FROM `tabSync Seq Counter` WHERE name = %s FOR UPDATE", "global"
	)[0][0]
	seq = current + 1
	frappe.db.sql(
		"UPDATE `tabSync Seq Counter` SET current_seq = %s WHERE name = %s", (seq, "global")
	)
