"""
Unit tests for spice_next_core.auth.remote_auth_client, the Phase 2 branch of
...
from spice_next_core.auth.decorators import current_remote_user_id, require_remote_auth, whitelist
...
class TestRemoteAuthClient(unittest.TestCase):
	...
	@patch("requests.post")
	def test_success_forwards_tenant_id_header(self, mock_post):
		mock_post.return_value = MagicMock(
			status_code=200,
			json=lambda: {"userDetail": {"username": "ext-user-tenant"}},
		)
		self.client.validate("token-g-tenant", tenant_id="20")
		_, kwargs = mock_post.call_args
		self.assertEqual(kwargs["headers"]["tenantId"], "20")

	@patch("requests.post")
	def test_no_tenant_id_omits_header(self, mock_post):
		mock_post.return_value = MagicMock(
			status_code=200,
			json=lambda: {"userDetail": {"username": "ext-user-no-tenant"}},
		)
		self.client.validate("token-h-no-tenant")
		_, kwargs = mock_post.call_args
		self.assertNotIn("tenantId", kwargs["headers"])

	@patch("requests.post")
	def test_inactive_user_raises(self, mock_post):
		mock_post.return_value = MagicMock(
			status_code=200,
			json=lambda: {"userDetail": {"username": "ext-user-1", "active": False}},
		)
		with self.assertRaises(RemoteAuthError):
			self.client.validate("token-inactive")
...
class TestJWTTokenValidatorPhase2(unittest.TestCase):
	...
	def test_remote_url_delegates_to_remote_client(self, mock_validate):
		...
		user_id = validator.extract_user_id("Bearer some-jwe-token", tenant_id="20")
		self.assertEqual(user_id, "ext-user-legacy-1")
		mock_validate.assert_called_once_with("some-jwe-token", "20")
...
class TestRequireRemoteAuthDecorator(unittest.TestCase):
	...
	@patch("spice_next_core.auth.decorators._build_validator")
